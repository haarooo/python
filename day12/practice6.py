...
print(s1+s2)
# a와c에서 결측치 발생 , b라는 인덱스는 s1,s2 모두 존재하는데 a와c라는 인덱스는 공통으로 존재하지 않음 
...
result1 = data.sort_values(ascending=False)
print(result1)
result2 = result.sort_index()
print(result2)
# 1차 정렬 이후에 유지 , kind속성에 'stable' 적용
# .sort(2차정렬).sort(1차정렬)
...
new_s1 = data[data>25]
print(new_s1)
new_s1 = new_s1.reset_index(drop=True)
print(new_s1)