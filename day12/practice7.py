...
data = pd.DataFrame(x , columns=['Model' , 'Price' , 'Company'])
print(data.info())
...
print(data.loc[['B', 'C'], ['Name', 'Age']] , data.iloc[1,2])
...
data.groupby('Category')['Values'].agg(['sum' , 'mean' ,'count'])
...
data.groupby(['Category','Type'])['Values'].mean()
