...
plt.plot(x,y , color='blue' , label = '매출' , linestyle=':')
plt.plot(x,y2 , color='red' , label = '비용' , linestyle='-')
plt.legend()
plt.grid()
plt.title('월별 실적 분석')
plt.show()
...
plt.bar(x - 0.2, 국어, width=0.4, label='국어')
plt.bar(x + 0.2, 수학, width=0.4, label='수학')
...
explodes = [0.1 , 0 ,0 ,0]
plt.pie(sizes , labels=labels  , colors=colors , explode=explodes , startangle=90 , autopct='%1.1f%%')
plt.legend()
plt.show()
...
x=[1, 2, 3, 4]
y=[10, 15, 13, 18]
plt.scatter(x,y , c='skyblue' , s=100)
plt.show()

data = [1, 2, 2, 3, 3, 3, 4, 4, 5]
plt.hist(data , bins=5 , c='skyblue')
plt.show()
...
# 5행 4열의 리스트 데이터 [[50, 60, 70, 80], [10, 20, 30, 40], [90, 80, 70, 60], [30, 40, 50, 60],[20, 10, 5, 0]
# 활용하여 히트맵을 그리시오.
...
df = pd.DataFrame({'매출': [100, 200, 150, 300, 250], '영업이익': [20, 50, 30, 80, 60]})
sns.boxenplot(data=df , fill=False , gap=0.1)
...
district = df.set_index('지역').select_dtypes(include=['number'])
sns.heatmap(district , cmap='coolwarm' , linewidths=0.5)
plt.show()     
